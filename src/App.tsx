...
function AdminPage({ navigate, session, onSignOut }: { navigate: (path: string) => void, session: AuthSession, onSignOut: () => void }) {
  ...
  const [newUserBoards, setNewUserBoards] = useState<string[]>([])
  ...
  const addUser = async () => {
    ...
    setStatus('Creating user and sending invitation…')
    try {
      let created = await inviteOrganizationUser(userEmail, session.idToken)
      if (newUserAdmin) created = await setUserAdmin(created.userId, true, session.idToken)
      for (const boardId of newUserBoards) created = await setUserModerator(boardId, created.userId, true, session.idToken)
      setUsers(current => [...current.filter(user => user.userId !== created.userId), created])
      setStatus(`User created. Cognito sent a temporary-password invitation to ${userEmail}.`)
      setEmail(''); setNewUserAdmin(false); setNewUserBoards([]); setCreatingUser(false)
    } catch (reason) { setStatus(reason instanceof Error ? reason.message : 'Could not create user.') }
  }
  const toggleNewUserBoard = (boardId: string, enabled: boolean) => setNewUserBoards(current => enabled ? [...current, boardId] : current.filter(id => id !== boardId))
  const validNewUserEmail = /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email.trim())
  ...
  return <div className="settings-shell"><header><button className="brand brand-button" onClick={() => navigate('/')}><span><Sparkles size={20} /></span> AMA Board</button><span className="admin-label">Site administration</span><button className="logout-button" onClick={onSignOut}><LogOut size={16} /> Log out</button><span className="profile">{session.email.slice(0, 2).toUpperCase()}</span></header>
    <main className="settings-page"><div className="settings-title"><div><span>Organisation console</span><h1>Administration</h1><p>Manage boards, users, and site-wide defaults.</p></div><button onClick={() => { setTab('Boards'); setCreating(true) }}><Plus size={17} /> Create board</button></div>
      <section className="settings-grid"><aside>{(['Overview', 'Boards', 'Users', 'Organisation'] as const).map(item => <button key={item} className={tab === item ? 'active' : ''} onClick={() => setTab(item)}>{item}</button>)}</aside><div className="settings-panels">
        ...
        {tab === 'Users' && <article><div className="panel-heading"><div><h2>Users and roles</h2><p>All Cognito users are shown here. Create members, grant organisation administrator access, or assign board-specific moderator access.</p></div><button onClick={() => { setStatus(''); setCreatingUser(true) }}><Plus size={16} /> Create user</button></div>{status && <p role="status">{status}</p>}{usersStatus && <p role="status">{usersStatus}</p>}<div className="user-list">{users.map(user => <div className="user-admin-row" key={user.userId}><span className="profile">{user.email.slice(0, 2).toUpperCase()}</span><div><b>{user.email}</b><small>{user.status.replaceAll('_', ' ').toLowerCase()} · {user.enabled ? 'enabled' : 'disabled'}</small><span className="role-badges">{user.isAdmin && <i>Administrator</i>}{user.moderatedBoardIds.length > 0 && <i>{user.moderatedBoardIds.length} moderated {user.moderatedBoardIds.length === 1 ? 'board' : 'boards'}</i>}{!user.isAdmin && user.moderatedBoardIds.length === 0 && <i>Member</i>}</span></div><button onClick={() => { setUserToManage(user); setRoleStatus('') }}><UserCog size={15} /> Manage roles</button></div>)}</div>{creatingUser && <div className="modal-backdrop" onMouseDown={() => setCreatingUser(false)}><section className="role-dialog create-user-dialog" role="dialog" aria-modal="true" aria-labelledby="create-user-title" onMouseDown={event => event.stopPropagation()}><button className="modal-close" onClick={() => setCreatingUser(false)} aria-label="Close"><X size={20} /></button><span className="composer-icon"><UserCog size={22} /></span><h2 id="create-user-title">Create organisation user</h2><p>Cognito will create the account and email the user a temporary password. They must choose a permanent password at first sign-in.</p><label>Email address <small>Required · use a complete organisation email address</small><input autoFocus required type="email" value={email} onChange={event => setEmail(event.target.value)} placeholder="new.user@company.com" /></label><div className="role-section"><div><b>Organisation administrator</b><span>Can manage all users, boards, roles, and organisation settings.</span></div><button className={`switch ${newUserAdmin ? 'on' : ''}`} onClick={() => setNewUserAdmin(!newUserAdmin)} aria-label="Create as organisation administrator"><i /></button></div><div className="moderated-boards"><h3>Board moderator access</h3><p>Optional. Select any boards this user should moderate immediately.</p>{boards.map(board => <label key={board.id}><input type="checkbox" checked={newUserBoards.includes(board.id)} onChange={event => toggleNewUserBoard(board.id, event.target.checked)} /><span><b>{board.title}</b><small>{board.visibility.toLowerCase()}</small></span></label>)}{boards.length === 0 && <p>No boards are available.</p>}</div>{status && <p role="status">{status}</p>}<div className="create-user-actions"><button onClick={() => setCreatingUser(false)}>Cancel</button><button disabled={!validNewUserEmail || status.startsWith('Creating')} onClick={addUser}>Create user and send invitation</button></div></section></div>}</article>}
