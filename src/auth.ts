...
export interface PersistedBoard { id: string, title: string, description?: string, visibility: string, postingPolicy: string, votingMode: string, commentsEnabled: boolean, visibleVoteTotals: boolean, anonymousPosting: boolean, categories: string[], presentedQuestionId?: string }
...
export const postQuestion = async (boardId: string, body: string, category: string, token?: string, identifyAs?: string) => (await graphQL<{ createQuestion: PersistedQuestion }>('mutation Post($input: CreateQuestionInput!) { createQuestion(input: $input) { id boardId body authorDisplayName category status upvotes downvotes createdAt comments { id authorDisplayName body createdAt questionId } } }', { input: { boardId, body, category, identifyAs } }, token)).createQuestion
