...
export interface PersistedBoard { id: string, title: string, description?: string, visibility: string, postingPolicy: string, votingMode: string, commentsEnabled: boolean, visibleVoteTotals: boolean, anonymousPosting: boolean, canModerate: boolean, presentedQuestionId?: string }

export const getBoard = async (id: string, token?: string) => (await graphQL<{ getBoard: PersistedBoard }>('query Board($id: ID!) { getBoard(id: $id) { id title description visibility postingPolicy votingMode commentsEnabled visibleVoteTotals anonymousPosting canModerate presentedQuestionId } }', { id }, token)).getBoard
