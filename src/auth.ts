...
export interface PersistedQuestion { id: string, boardId: string, body: string, authorDisplayName: string, category: string, status: string, upvotes: number, downvotes: number, comments: PersistedComment[], createdAt: string }
export interface PersistedBoard { id: string, title: string, description?: string, visibility: string, postingPolicy: string, votingMode: string, commentsEnabled: boolean, visibleVoteTotals: boolean, anonymousPosting: boolean, presentedQuestionId?: string }

export const getBoard = async (id: string, token?: string) => (await graphQL<{ getBoard: PersistedBoard }>('query Board($id: ID!) { getBoard(id: $id) { id title description visibility postingPolicy votingMode commentsEnabled visibleVoteTotals anonymousPosting presentedQuestionId } }', { id }, token)).getBoard
export const getQuestions = async (boardId: string, token?: string) => (await graphQL<{ listQuestions: { items: PersistedQuestion[] } }>('query Questions($boardId: ID!) { listQuestions(boardId: $boardId) { items { id boardId body authorDisplayName category status upvotes downvotes createdAt comments { id authorDisplayName body createdAt questionId } } } }', { boardId }, token)).listQuestions.items
