...
export async function inviteUser(boardId: string, email: string, idToken: string) {
  ...
  const response = await fetch(config.appSyncEndpoint, {
    ...
    body: JSON.stringify({ query: 'mutation Invite($boardId: ID!, $email: AWSEmail!) { inviteUser(boardId: $boardId, email: $email) { boardId userId email role invitationStatus } }', variables: { boardId, email } }),
  })
  const payload = await response.json()
...
export interface BoardMember { boardId: string, userId: string, email?: string, role: 'OWNER' | 'MODERATOR', invitationStatus?: 'SENT' | 'RESENT' | 'EXISTING' }
...
export const listBoardMembers = async (boardId: string, token: string) => (await graphQL<{ listBoardMembers: BoardMember[] }>('query BoardMembers($boardId: ID!) { listBoardMembers(boardId: $boardId) { boardId userId email role invitationStatus } }', { boardId }, token)).listBoardMembers
